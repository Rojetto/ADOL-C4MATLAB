...
void acccov(int n, int d, double ***B, double **C, double **D)
{
	...
	// D_k:=Fak*C_k (1. Summationsschritt)
	for (k = 0; k <= d; k++)
	{
		if(k == 0)
			Fak = 1;
...
void accadj(int n, int deg, double ***A, double ***Bs)
{
	...
	// (1. Summationsschritt)
	//double ***At = myalloc3(n,n,deg+1); // jcw: überflüssig
	...
	for (k = 0; k <= deg; k++)
	{
		for (j = 0; j < n; j++)
		{
			for (i = 0; i < n; i++)
			{
				//At[j][i][k] = A[i][j][k];          // jcw: überflüssig
				//Bs[j][i][k] = -At[j][i][k]/(k+1);  // jcw: überflüssig
			...
			for (l = 1; l <= k; l++)
			{
				for (jp = 0; jp < n; jp++)
				{
					for (ip = 0; ip < n; ip++)
					{
						...
						for (kp = 0; kp < n; kp++)
						{
							//x+=At[jp][kp][k-l]*Bs[kp][ip][l-1]; // jcw: überflüssig
...
{
	int i, j, k, l, jp, kp;
	//double ***Bst = myalloc3(n,n,d+1); // jcw überflüssig wegen Vereinfachungen, siehe (A)
	...
	for (k = 0; k <= d; k++)
	{
		...
		for (j = 0; j < n; j++)
		{
			for (i = 0; i < n; i++)
			{
				//Bst[i][j][k] = Bs[j][i][k]; // jcw: überflüssig (A)
			...
			if(k >= 1)
			{
				...
				for (l = 1; l <= k; l++)
				{
					for (jp = 0; jp < n; jp++)
					{
						...
						for (kp = 0; kp < n; kp++)
						{
							//x+=Bst[jp][kp][l-1]*b[kp][k-l]; // jcw: Überflüssig (A)
							
							// jcw (A): so gehts direkt
...
{
	double** X = myalloc2(n, d+1);   // Taylorkoeff. Entwicklung x(t)
	double** Y = myalloc2(m, d+1);   // Taylorkoeff. Entwicklung y(t)
	double*  x = myalloc1(n);
	double*  y = myalloc1(m);
	...
	for (i = 0; i < n; i++) 
	{
		x[i] = X[i][0]; // jcw überflüssig, stattdessen x0 unten bei hos_forward verwenden ?
		for (k = 0; k < d; k++)
			X[i][k] = X[i][k+1];
...
int lie_gradientcv(short Tape_F, short Tape_H, short n,	short m, double* x0, short d, double*** result) 
{
	...
	hov_reverse(Tape_F,n,n,d-1,n,In,A,0);// Erklaerung in interfaces.cpp
	accodec(n, 1.0, d-1, A, B, 0);      // Erklaerung in odedrivers.c
...
{
	int m = n;                     
	double  **X  = myalloc2(n, d+2);   // Taylorkoeff. Entwicklung x(t)
	double  **Y  = myalloc2(m, d+2);   // Taylorkoeff. Entwicklung y(t)
	...
	// Einheitsmatrix für hov_reverse, statisch, damit nicht immer neue angelegt
